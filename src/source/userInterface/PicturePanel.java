...
import source.model.DrawableShape;
...
public class PicturePanel extends JPanel {
    ...
    private final source.userInterface.Canvas canvas;

    public PicturePanel(int sizeX, int sizeY) {
        canvas = new source.userInterface.Canvas(sizeX, sizeY);
    ...
    /**
     * An Icon which renders a Canvas into a graphical form. Pass the constructor the Canvas you want to render.
    ...
    private static class CanvasIcon implements Icon {
        ...
        /* Create the CanvasIcon from an existing Canvas with characters and coloured blocks in it. */
        ...
        public void paintIcon(Component c, Graphics g, int xPos, int yPos) {
            // take the basic "Graphics" context and convert to more powerful Graphics2D context.
            ...
            for (int y = 0; y < canvas.getSizeY(); y++) {
                for (int x = 0; x < canvas.getSizeX(); x++) {
                    ...
                    g2d.drawString("" + canvas.getCellText(x, y), left + TEXT_OFFSET_X, top + TEXT_OFFSET_Y);
                }
            }
        }
    }
